...
> **AUDIENCE SPLIT RULE (per MJ):** a LakbayHub record belongs to **AACIO** (and is **excluded from every POTB view** — Acquisition, Insights, Operations, Bookings auto-fill) if EITHER its cluster name contains "aacio" or "external" (case-insensitive, `isExternalCluster`) OR it used a known AACIO payment link (`AACIO_PAYMENT_LINK_IDS`, matched against `meta.payment_link`). Combined predicate `isExternalRecord` in `api/lakbayhub.js`. The payment-link list is a safety net for sales whose cluster wasn't tagged AACIO on the LakbayHub side. The split happens at the source in `lakbay.js` `fetchFresh` — `fetchSalesRecords()` returns POTB-only; AACIO reads `getExternalSalesRecords()`.
