...
6. **AACIO** — Acquisition-style sales monitoring for the **external team** (second YCBM account). Bookings here ARE their sales source. KPIs (Total Bookings / Active Sales / Unique Leads / Avg per Day), daily trend, time-slot bar, bookings table + CSV. **Sales side**: rides the shared `lakbay.js` pipeline — `fetchSalesRecords()` warms the cache, `getExternalSalesRecords()` reads the `EXTERNAL COACH - *` split (one API hit serves both audiences) — then renders the same presentational blocks the Acquisition tab uses: NeedsReview, TodaySnapshot, FunnelHealth, PackagePerformance, LiveActivityFeed + ClusterHealth, plus DataSourceBanner. (TargetProgress/SmartInsights deliberately excluded — they pace vs the global POTB ₱1M target.) Own YCBM data source (`useAacioData` → `api/ycbmAacio.js` → `/api/aacio` proxy) so it renders independently of POTB's slow load. Files: `components/AacioReportTab.jsx`, `hooks/useAacioData.js`, `api/ycbmAacio.js`.

> **AUDIENCE SPLIT RULE (per MJ):** a LakbayHub record belongs to **AACIO** (and is **excluded from every POTB view** — Acquisition, Insights, Operations, Bookings auto-fill) purely by **cluster prefix**: its `cluster_name` contains "aacio" or "external" (case-insensitive, `isExternalCluster` = `isExternalRecord`). The SAME coach does both jobs, tagged distinctly: `AACIO <coach>` / `EXTERNAL COACH - <coach>` → AACIO; `ACQUISITION - <coach>` (and `2GO - FREE ACCOUNT`) → POTB. The old `AACIO_PAYMENT_LINK_IDS` payment-link override was **removed** (clusters are cleanly tagged now; it was mis-moving legit `ACQUISITION -` sales to AACIO). The split happens at the source in `lakbay.js` `fetchFresh` — `fetchSalesRecords()` returns POTB-only; AACIO reads `getExternalSalesRecords()`.
>
> **`sales_closer` is blank on ~100% of LakbayHub records** — the real closer is in the cluster (`ACQUISITION - MARTIN` → `Martin`). `coachFromCluster()` in `api/lakbayhub.js` extracts it; `mapLakbayHubRecord` falls back to it for `sales_agent` so Top Closer / per-agent show real names instead of "Unassigned". **Absurd-amount guard:** `amount_paid > ₱1,000,000` → counted as 0 + flagged (a ₱30B Princess typo was blowing up totals).
7. **Insights → Reports** — Comprehensive LakbayHub reports hub (sign-ups + by closer/package + officers + **AACIO**) with range filter, charts + CSV export. (sub-view of Insights) The **AACIO** category reads the external split via `getExternalSalesRecords()` (dated only) — KPI strip + by-cluster breakdown + detailed list, range-filtered like the others. Kept separate from the POTB sign-up/closer/package reports.
