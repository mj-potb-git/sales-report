...
// Bars: Revenue + Profit per bucket (both PHP). Used by the Officers tab.
...
export default function RevenueTrend({ records = [] }) {
  ...
  const data = useMemo(() => {
    ...
    for (const r of records) {
      ...
      if (!m.has(key)) m.set(key, { key, label, Revenue: 0, Profit: 0 })
      const g = m.get(key)
    ...
    if (gran === 'month') {
      ...
      for (let i = 0; i <= d.getMonth(); i++) {
        ...
        out.push(m.get(key) || { key, label: `${MONTHS[i]} '${String(yr).slice(2)}`, Revenue: 0, Profit: 0 })
      }
      return out
    }
    ...
    <section className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-[11px] text-gray-500 mt-0.5">Revenue + Profit · comparison across time (lahat ng data)</p>
        </div>
